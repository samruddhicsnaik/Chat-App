...
public class ReceivedRequestsFragment extends Fragment
{
    ...
    private DatabaseReference chatRequestsRef, friendsRef;
    ...
    {
        view = inflater.inflate(R.layout.fragment_received_requests, container, false);
        firebaseAuth = FirebaseAuth.getInstance();
        currentUserID = firebaseAuth.getCurrentUser().getUid();
        chatRequestsRef = FirebaseDatabase.getInstance().getReference().child("Chat Requests");
        friendsRef = FirebaseDatabase.getInstance().getReference().child("Friends");
        recyclerView = view.findViewById(R.id.received_chat_requests_list);
    ...
    public void onStart()
    {
        ...
        FirebaseRecyclerOptions<People> options = new FirebaseRecyclerOptions.Builder<People>()
                .setQuery(chatRequestsRef.child(currentUserID).child("received"), People.class)
                .build();
                    ...
                    protected void onBindViewHolder(@NonNull final RequestsViewHolder requestsViewHolder, int i, @NonNull People people) {
                        ...
                        requestsViewHolder.accept.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                chatRequestsRef.child(currentUserID).child("received").child(userID).removeValue()
                                        .addOnCompleteListener(new OnCompleteListener<Void>()
                                        {
                                            ...
                                            public void onComplete(@NonNull Task<Void> task)
                                            {
                                                ...
                                                {
                                                    chatRequestsRef.child(userID).child("sent").child(currentUserID).removeValue()
                                                            .addOnCompleteListener(new OnCompleteListener<Void>()
                                                            {
                                                                @Override
                                                                public void onComplete(@NonNull Task<Void> task)
                                                                {
                                                                    friendsRef.child(currentUserID).child("friends").child(userID).child("request type").setValue("friends")
                                                                            .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                @Override
                                                                                public void onComplete(@NonNull Task<Void> task) {
                                                                                    friendsRef.child(userID).child("friends").child(currentUserID).child("request type").setValue("friends");
                                                                                }
                                                                            });
                                                                }
                                                            });
